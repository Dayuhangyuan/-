...
public class ScDao {
	...
	public int[] add(List<Integer> deleteArr,List<Integer> addArr,Integer stuId) throws SQLException {
		DataSource dataSource = PropertiesUtils.getDataSource();
		Connection connection = dataSource.getConnection();
		connection.setAutoCommit(false);
		QueryRunner queryRunner = new QueryRunner(dataSource);
		if (deleteArr.size() > 0) {
			String _sql = "delete from sc where stuId = ? and cId in (";
		for(int i = 0;i < deleteArr.size();i++){
			_sql += "'" + deleteArr.get(i) + "'";
			if (i < deleteArr.size() - 1) {
				_sql += ",";
			}
		}
		_sql += ")";
		...
		int[] arr = null;
		if (addArr.size() > 0) {
			Object[][] object = new Object[addArr.size()][2];
		//将cIdArray与stuID保存为二维数组
		for(int i=0;i<addArr.size();i++) {
			object[i][0] = stuId;
			object[i][1] = addArr.get(i);
		}
		String sql = "insert into sc(stuId,cid) values(?,?)";
		arr = queryRunner.batch(connection,sql, object);
		}
		connection.commit();
		return arr;
	}
